...
## Description
Health domain package. Health metrics types, health goals, and tracking models.
...
```bash
pnpm --filter @medivo/health dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/health build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/health test
```
