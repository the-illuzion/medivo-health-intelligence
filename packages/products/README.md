...
## Description
Product domain package. Product types, category models, inventory interfaces.
...
```bash
pnpm --filter @medivo/products dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/products build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/products test
```
