...
## Description
Authentication utilities. JWT handling, auth hooks, auth context, protected routes.
...
```bash
pnpm --filter @medivo/auth dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/auth build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/auth test
```
