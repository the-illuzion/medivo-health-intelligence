...
## Description
Type-safe HTTP client with interceptors, error handling, and retry logic.
...
```bash
pnpm --filter @medivo/api-client dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/api-client build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/api-client test
```
