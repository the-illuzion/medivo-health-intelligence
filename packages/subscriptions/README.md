...
## Description
Subscription management package. Plan types, billing cycles, feature gating.
...
```bash
pnpm --filter @medivo/subscriptions dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/subscriptions build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/subscriptions test
```
