...
## Description
Analytics package. Event tracking, metric types, analytics utilities.
...
```bash
pnpm --filter @medivo/analytics dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/analytics build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/analytics test
```
