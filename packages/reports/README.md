...
## Description
Report generation package. Report templates, PDF generation, and chart data formatting.
...
```bash
pnpm --filter @medivo/reports dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/reports build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/reports test
```
