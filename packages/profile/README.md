...
## Description
User profile package. Profile types, preference models, health profile interfaces.
...
```bash
pnpm --filter @medivo/profile dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/profile build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/profile test
```
