# E-Commerce Shared Utilities

## Description
E-commerce shared utilities. Cart logic, pricing calculations, discount rules.
...
```bash
pnpm --filter @medivo/ecommerce dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/ecommerce build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/ecommerce test
```
