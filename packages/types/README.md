...
## Description
Shared TypeScript types. Base types, domain interfaces, API response types. LEAF DEPENDENCY — no internal dependencies.
...
```bash
pnpm --filter @medivo/types dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/types build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/types test
```
