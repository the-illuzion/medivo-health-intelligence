...
## Description
Checkout flow package. Checkout steps, payment integration types, order creation.
...
```bash
pnpm --filter @medivo/checkout dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/checkout build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/checkout test
```
