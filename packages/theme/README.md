...
## Description
Theme configuration and utilities. TailwindCSS theme extension, CSS custom properties, dark/light mode.
...
```bash
pnpm --filter @medivo/theme dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/theme build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/theme test
```
