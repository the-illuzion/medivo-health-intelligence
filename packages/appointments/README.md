...
## Description
Appointment domain package. Appointment types, scheduling logic, availability models.
...
```bash
pnpm --filter @medivo/appointments dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/appointments build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/appointments test
```
