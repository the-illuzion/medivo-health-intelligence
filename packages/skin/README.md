...
## Description
Skin intelligence domain package. Skin analysis types, skin condition models, routine types.
...
```bash
pnpm --filter @medivo/skin dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/skin build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/skin test
```
