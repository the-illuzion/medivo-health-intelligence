...
## Description
Customer-facing Progressive Web Application. Responsive PWA offering AI skin analysis, health reports, coaching, routines, marketplace, and consultations.
...
## Technology Stack
- **Core**: Next.js, TypeScript, TailwindCSS
- **Dependencies**: react, next, shadcn-ui
...
```bash
pnpm --filter @medivo/customer-platform dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/customer-platform build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/customer-platform test
```
