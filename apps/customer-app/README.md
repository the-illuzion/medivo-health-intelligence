...
## Description
Native mobile application built with React Native and Expo. Features include camera for skin analysis, biometrics, push notifications, and HealthKit/Health Connect integration.
...
```bash
pnpm --filter @medivo/mobile dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/mobile build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/mobile test
```
