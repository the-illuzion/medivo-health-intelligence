...
## Description
Backend-For-Frontend serving the doctor-portal. Aggregates APIs tailored for doctor workflows.
...
```bash
pnpm --filter @medivo/doctor-bff dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/doctor-bff build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/doctor-bff test
```
