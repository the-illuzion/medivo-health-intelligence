...
## Description
Internal administration panel for user management, product management, analytics, and system configuration.
...
```bash
pnpm --filter @medivo/admin-panel dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/admin-panel build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/admin-panel test
```
