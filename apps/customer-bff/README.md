...
## Description
Backend-For-Frontend serving the customer-platform and mobile app. Aggregates and optimizes API calls for the best customer experience.
...
```bash
pnpm --filter @medivo/customer-bff dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/customer-bff build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/customer-bff test
```
