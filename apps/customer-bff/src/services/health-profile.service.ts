...
class HealthProfileService {
  ...
  private createDefaultProfile(userId: string, userSession?: { name?: string; email?: string }): HealthProfileData {
    ...
    return {
      userId,
      name: userSession?.name || 'New Member',
      age: 0,
  ...
  public getProfile(userId: string, userSession?: { name?: string; email?: string }): HealthProfileData {
    if (!this.profiles.has(userId)) {
      this.profiles.set(userId, this.createDefaultProfile(userId, userSession));
    }
