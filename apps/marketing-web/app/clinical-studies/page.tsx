...
import { Award, CheckCircle2, ShieldCheck, Activity } from 'lucide-react';
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-16 space-y-16">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-4">
        <div className="inline-flex items-center gap-2 bg-emerald-950/60 border border-emerald-500/30 text-emerald-400 text-xs font-extrabold px-4 py-2 rounded-2xl">
          <Award className="w-4 h-4" />
          <span>PEER-REVIEWED CLINICAL VALIDATION</span>
        </div>
        <h1 className="text-4xl sm:text-5xl font-extrabold text-white">99.4% Clinical Diagnostic Accuracy</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {benchmarks.map((b, idx) => (
          <div key={idx} className="bg-[#111827] border border-[#1f2937] p-6 rounded-3xl space-y-3">
            <span className="text-4xl font-extrabold text-emerald-400">{b.score}</span>
            <h3 className="text-sm font-bold text-white">{b.metric}</h3>
            <p className="text-xs text-slate-400">{b.benchmark}</p>
          </div>
        ))}
      </div>

      {/* Technology Deep Dive */}
      <div className="bg-[#111827] border border-[#1f2937] p-8 sm:p-12 rounded-3xl space-y-8">
        <h2 className="text-2xl font-extrabold text-white">Multi-Spectral Computer Vision Pipeline</h2>
