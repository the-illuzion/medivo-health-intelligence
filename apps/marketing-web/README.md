...
## Description
Public marketing website. Next.js with SSG. Landing pages, blog, pricing, about.
...
```bash
pnpm --filter @medivo/marketing-web dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/marketing-web build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/marketing-web test
```
