...
## Description
Notification delivery service for Email, SMS, push notifications, and in-app notifications. Manages templates.
...
```bash
pnpm --filter @medivo/service-notifications dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/service-notifications build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/service-notifications test
```
