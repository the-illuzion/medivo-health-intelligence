...
## Description
Appointment management service for scheduling, availability, reminders, and consultation management.
...
```bash
pnpm --filter @medivo/service-appointments dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/service-appointments build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/service-appointments test
```
