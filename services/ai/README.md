...
## Description
AI processing service handling model inference, skin analysis pipelines, health predictions, and report generation.
...
```bash
pnpm --filter @medivo/service-ai dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/service-ai build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/service-ai test
```
