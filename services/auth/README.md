...
## Description
Authentication and authorization service for JWT issuance, RBAC, session management, and OAuth providers.
...
```bash
pnpm --filter @medivo/service-auth dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/service-auth build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/service-auth test
```
