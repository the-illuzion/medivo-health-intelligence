...
## Description
E-commerce service for product catalog, cart management, checkout, payment processing, and order fulfillment.
...
```bash
pnpm --filter @medivo/service-commerce dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/service-commerce build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/service-commerce test
```
