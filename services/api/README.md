...
## Description
Modular monolith. Node.js + TypeScript + Clean Architecture. Contains domain modules for all business logic. PostgreSQL with schema-per-domain.
...
## Technology Stack
- **Core**: Node.js, TypeScript, PostgreSQL
...
```bash
pnpm --filter @medivo/service-api dev
```

### Building
To build this package for production:
```bash
pnpm --filter @medivo/service-api build
```

### Testing
Run the test suite specifically for this module:
```bash
pnpm --filter @medivo/service-api test
```
